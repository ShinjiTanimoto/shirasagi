module Guide2::Importer::Question
    ...
    Enumerator.new do |y|
      ...
      if edge_size
        edge_size.times do |i|
          headers << "#{I18n.t("guide2.transition")}#{i + 1}"
          headers << "#{I18n.t("guide2.explanation")}#{i + 1}"
        end
      end
  ...
  def save_question
    ...
    if question_type == "yes_no"
      ...
      in_edges[idx][:value] = I18n.t("guide2.links.applicable")

      # no
      idx = 1
      in_edges[idx] = OpenStruct.new
      in_edges[idx][:question_type] = question_type
      in_edges[idx][:value] = I18n.t("guide2.links.not_applicable")
