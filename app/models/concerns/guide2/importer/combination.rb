module Guide2::Importer::Combination
  ...
  def combinations_enum
    Enumerator.new do |y|
      headers = [I18n.t("guide2.procedure")] # 手続き
      ...
      headers = [I18n.t("guide2.question")] # 質問
      y << encode_sjis(headers.to_csv)

      headers = ['']
      headers += %w(id_name name explanation order question_type check_type).map { |v| Guide2::Question.t(v) }
      headers << I18n.t("guide2.transition")
      headers << I18n.t("guide2.explanation")
      headers << I18n.t("guide2.transition_settings")
      y << encode_sjis(headers.to_csv)
      ...
      Guide2::Question.site(cur_site).node(cur_node).each do |item|
        ...
        item.edges.to_a.each do |edge|
          ...
          edge_list = edge.points.map do |point|
            ...
            label = edge.not_applicable_points.pluck(:id).include?(point.id) ? I18n.t('guide2.labels.not_applicable') : nil
            value = value.sub('] ', "][#{label}] ") if label
            # necessary or optional_necessary
            label = edge.optional_necessary_points.pluck(:id).include?(point.id) ? I18n.t('guide2.labels.optional_necessary') : nil
            value = value.sub('] ', "][#{label}] ") if label
            value
          end
          row << edge_list.join("\n")
  ...
  def import_combinations
    ...
    self.class.each_csv(in_file) do |row, index|
      @row_index += 1
      next if row[0] != I18n.t("guide2.question")
    ...
    @questions.each do |item|
      break if save_combination_procedure_edges(item) == false
    end
  ...
  def save_combination_procedure
    ...
    if item.save
      ...
      errors.add :base, "Line #{@row_index}: #{I18n.t("guide2.errors.save_failed", message: message)}"
      false
    end
  end
  ...
  def save_combination_question
    ...
    if @row[1].present? # 基本情報
      question_types = Guide2::Question.new.question_type_options.to_h
      check_types = Guide2::Question.new.check_type_options.to_h

      item = Guide2::Question.site(cur_site).node(cur_node).where(id_name: id_name).first
      item ||= Guide2::Question.new
      item.cur_site = cur_site
      item.cur_node = cur_node
      item.cur_user = cur_user
      item.id_name = id_name
  ...
  def save_combination_question_edges(item)
    label_question = I18n.t('guide2.question')
    label_procedure = I18n.t('guide2.procedure')
    label_not_applicable = I18n.t('guide2.labels.not_applicable')
    label_optional_necessary = I18n.t('guide2.labels.optional_necessary')
    ...
    item.in_edges = item.tmp_edges.to_a.map do |edge|
      ...
      edge[:point_names].to_a.each do |line|
        ...
        if cate.match(label_procedure)
          rel = Guide2::Procedure.site(cur_site).node(cur_node).where(id_name: name).first
          # edge[:point_ids] << rel.id if rel
          # item.errors.add :base, I18n.t('guide2.errors.not_found_procedure', id: name) unless rel
        elsif cate.match(label_question)
          rel = Guide2::Question.site(cur_site).node(cur_node).where(id_name: name).first
          # edge[:point_ids] << rel.id if rel
          # item.errors.add :base, I18n.t('guide2.errors.not_found_question', id: name) unless rel
        end
    ...
    if item.errors.empty?
      ...
      errors.add :base, "Line #{item.row_index}: #{I18n.t("guide2.errors.save_failed", message: message)}"
      false
    end
  end
  # rubocop:enable Metrics/PerceivedComplexity

  def save_combination_procedure_edges(item)
    label_question = I18n.t('guide2.question')
    label_procedure = I18n.t('guide2.procedure')
    label_not_applicable = I18n.t('guide2.labels.not_applicable')
    label_optional_necessary = I18n.t('guide2.labels.optional_necessary')

    item.tmp_edges.each do |edge|
      # 非該当, 任意必要
      edge[:point_names].to_a.each do |line|
        cate, name = line.split(/ /, 2)
        rel = nil
        ...
        if cate.match(label_procedure)
          rel = Guide2::Procedure.site(cur_site).node(cur_node).where(id_name: name).first
          edge_id = item.edges.where(value: edge[:value]).pluck(:id).first.to_s
          rel.add_to_set(cond_yes_question_ids: item.id, cond_yes_edge_values: { question_id: item.id.to_s, edge_value: edge[:value] })
          # edge[:point_ids] << rel.id if rel
          # item.errors.add :base, I18n.t('guide2.errors.not_found_procedure', id: name) unless rel
        elsif cate.match(label_question)
          rel = Guide2::Question.site(cur_site).node(cur_node).where(id_name: name).first
        ...
        if rel && cate.match(label_not_applicable)
          edge_id = item.edges.where(value: edge[:value]).pluck(:id).first.to_s
          rel.add_to_set(cond_no_question_ids: item.id, cond_no_edge_values: { question_id: item.id.to_s, edge_value: edge[:value] })
          # edge[:not_applicable_point_ids] << rel.id
        end
        if rel && cate.match(label_optional_necessary)
          edge_id = item.edges.where(value: edge[:value]).pluck(:id).first.to_s
          rel.add_to_set(cond_or_question_ids: item.id, cond_or_edge_values: { question_id: item.id.to_s, edge_value: edge[:value] })
          # edge[:optional_necessary_point_ids] << rel.id
        end
      end
