module Guide2::Addon
  module Question
    ...
    included do
      ...
      embeds_many :edges, class_name: 'Guide2::Diagram::Edge'
    ...
    def question_type_options
      I18n.t("guide2.options.question_type").map { |k, v| [v, k] }
    end

    def check_type_options
      I18n.t("guide2.options.check_type").map { |k, v| [v, k] }
    end
    ...
    def validate_in_edges
      ...
      with_type.each_with_index do |in_edge, i|
        edge = ::Guide2::Diagram::Edge.new(in_edge)
        edge.parent = self
        next if edge.valid?

        edge.errors.full_messages.each do |msg|
          self.errors.add :base, "#{I18n.t("guide2.labels.choices")} #{i + 1} : #{msg}"
        end
      end
