class Guide2::Apis::QuestionsController < ApplicationController
  ...
  class Aggregator
    ...
    def call
      ...
      aggregation_view = Guide2::Question.collection.aggregate(@stages)
      result_doc = aggregation_view.first

      items = result_doc["paginatedResults"].map do |result|
        [
          Mongoid::Factory.from_db(Guide2::Question, result.except("nedges")),
          result["nedges"].blank? ? nil : Mongoid::Factory.from_db(Guide2::Diagram::Edge, result["nedges"])
        ]
      end
    ...
    def stage_default_selector
      default_selector = Guide2::Question.exists(question_type: true).selector
    ...
    def stage_site_selector
      site_selector = Guide2::Question.unscoped.site(@cur_site).selector
