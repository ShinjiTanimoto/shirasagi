class Guide2::Agents::Nodes::GuideController < ApplicationController
  include Cms::NodeFilter::View
  helper Guide2::ListHelper
  ...
  def set_answers
    ...
    if params[:question].present?
      question = ::Guide2::Question.where(id: params[:question]).first
      ...
      if params[:answers].present?
        if question_type == "choices"
          ...
          params[:answers].each do |no, transitions|
            if params[:submit] == I18n.t("guide2.links.applicable")
              transitions = [transitions[0]]
            elsif params[:submit] == I18n.t("guide2.links.not_applicable")
  ...
  def index
    @diagram = ::Guide2::QuestionDiagram.new @cur_node
  ...
  def result
    @cur_node.name = I18n.t("guide2.labels.result", name: @cur_node.name)
