class Guide2::MermaidComponent < ApplicationComponent
  ...
  def call
    ...
    output_buffer << tag.div(class: "mermaid guide-diagnostic-chart") do
      ...
      all_questions = all_points.select { |point| point.is_a?(Guide2::Question) }
      ...
      all_procedures = all_points.reject { |point| point.is_a?(Guide2::Question) }
  ...
  def all_points
    @all_points ||= Guide2::Diagram::Point.all.site(site).node(node).to_a
  end
  ...
  def build_point(point)
    ...
    begin
      if point.is_a?(Guide2::Question)
        @additional_info["ID#{point.id}"] = { url: guide2_question_path(site: point.site_id, cid: point.node_id, id: point.id) }
        "ID#{point.id}{{\"#{escape(point.name_with_type)}\"}}\n".html_safe
      else
        @additional_info["ID#{point.id}"] = { url: guide2_procedure_path(site: point.site_id, cid: point.node_id, id: point.id) }
        "ID#{point.id}[[\"#{escape(point.name_with_type)}\"]]\n".html_safe
      end
    ensure
  ...
  def build_edge(from_point, to_point, edge)
    @additional_info["L-L-ID#{from_point.id}-ID#{to_point.id}"] = {
      url: guide2_question_path(site: from_point.site_id, cid: from_point.node_id, id: from_point.id),
